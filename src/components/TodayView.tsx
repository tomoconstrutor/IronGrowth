...
import type { DailyUpdate } from "../lib/dailyUpdates";
...
export function TodayView({ weekNumber, workouts, logs, dailyUpdates, onRegister }: TodayViewProps) {
  ...
  const todayUpdate = dailyUpdates.find((update) => update.date === today);
    ...
    <div className="today-view">
      ...
      <aside className="today-editorial">
        ...
        <section className="discipline-strips" aria-label="Progress by sport">
          ...
          <div className="discipline-strip nutrition-strip">
            <span>CALORIES</span>
            <div><i style={{ width: todayUpdate?.caloriesTarget ? "100%" : "0%" }} /></div>
            <strong>{todayUpdate?.caloriesTarget ?? "—"}<small> KCAL TARGET</small></strong>
          </div>
        </section>
      </aside>
    </div>
